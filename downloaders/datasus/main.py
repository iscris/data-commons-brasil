...
def get_page_content(url, max_retries=5, delay_seconds=20):
    ...
    while retries < max_retries:
        ...
        print(f"""Retrying {retries + 1}/
              {max_retries} after {delay_seconds} seconds...""")
        ...
        for link in links:
            if link.text != "»":
                ...
                for dataset in datasets:
                    heading = dataset.find(
                        "h3", class_="dataset-heading").find("a")
                    print(heading.text)
                            ...
                            if search_button is not None:
                                ...
                                if file_type == "api":
                                    ...
                                    if usuario is None or senha is None:
                                        apidata = get_page_content(
                                            item["href"])
                                        script_tags = apidata.find_all(
                                            'script')
                                        for script_tag in script_tags:
                                            javascript_code = (
                                                script_tag.string if
                                                script_tag else None)

                                            if (javascript_code and
                                                'var user_config' in
                                                    javascript_code):
                                    ...
                                    file_name = clean_filename(
                                        resource_item.text) + ".json"
                                    file_path = os.path.join(
                                        dataset_path, file_name)
                                    download_resource(
                                        item["href"], file_path, basic)
                                    usuario = None
                                    senha = None
                                else:
                                    file_name = clean_filename(
                                        resource_item.text) + "." + file_type
                                    file_path = os.path.join(
                                        dataset_path, file_name)
                                    download_resource(item["href"], file_path)
                        if file_type == "csv":
                            ...
                            if item is not None:
                                ...
                                file_path = os.path.join(
                                    dataset_path, file_name)
                                download_resource(
                                    item.find("a")["href"], file_path)
                            else:
                                csvlist = data.find(
                                    "div", class_="prose notes")
                                if csvlist is None:
                                    retries = 0
                                    while retries < 5:
                                        time.sleep(20)
                                        ...
                                        if csvlist is not None:
                                            retries = 5
                                            for csvlink in csvlist.find_all(
                                                    "a", href=True):
                                                csv_text = clean_filename(
                                                    csvlink.text)
                                                file_name = (csv_text +
                                                             "." + file_type)
                                                file_path = os.path.join(
